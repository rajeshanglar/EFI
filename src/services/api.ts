...
api.interceptors.request.use(async config => {
 ...
 // Check if this is a login, forgot password, or reset password endpoint
  ...
  // Check if this is a videos endpoint - requires user authentication
  const isVideosEndpoint = config.url?.includes('v1/videos') || config.url?.includes('/videos');
  
  // For videos endpoint: ONLY use user's accessToken (requires login)
  if (isVideosEndpoint) {
    ...
    if (accessToken) {
      ...
      console.error('=== VIDEOS ENDPOINT: NO ACCESS TOKEN AVAILABLE ===');
      console.error('Videos endpoint requires user to be logged in!');
      // Don't set Authorization header - let the API return 401
    }
    return config;
  }
  ...
  async error => {
    ...
    if (isVideosEndpoint) {
      ...
      if (status === 401) {
        ...
        console.error('The API token is no longer valid. User needs to log in again.');
        console.error('Clearing invalid tokens...');
        
        // Clear invalid tokens when 401 occurs
        try {
          await AsyncStorage.multiRemove(['accessToken', 'refreshToken', 'tokenExpiry', 'auth_token', 'user_data']);
          console.error('Invalid tokens cleared. User should log in again.');
        } catch (clearError) {
