...
interface DelegateListDetailsProps {
  onBack: () => void;
  onNavigateToHome: () => void;
  delegateData?: DelegateDetails;
}

const DelegateListDetails: React.FC<DelegateListDetailsProps> = ({
  onBack,
  onNavigateToHome,
  delegateData,
}) => {
  // Default delegate data if not provided
  const defaultDelegate: DelegateDetails = {
    id: '1',
    name: 'Gajbhiye, Rahul',
    titles: [
  ...
  const delegate = delegateData || defaultDelegate;
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        >
          <View style={styles.headerContent}>
            ...
            <View style={styles.delegateInfoContainer}>
              ...
              {delegate.titles.map((title, index) => (
                <Text key={index} style={styles.delegateTitle}>
                  {title}
                </Text>
              ))}
            </View>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.sectionText}>{delegate.biography}</Text>
          </View>

          {/* Session Section */}
          <View style={styles.section}>
            <Text style={styles.sectionHeading}>Session</Text>
            <Text style={styles.sessionTitle}>{delegate.sessionTitle}</Text>
            {delegate.sessionSubtitle && (
              <Text style={styles.sessionSubtitle}>{delegate.sessionSubtitle}</Text>
            )}
            ...
            <View style={styles.sessionMetadata}>
              <View style={styles.metadataItem}>
                ...
                <Text style={styles.metadataText}>{delegate.date}</Text>
              </View>
              <View style={styles.metadataItem}>
                <MapWIcon size={20} color={colors.primary} />
                <Text style={styles.metadataText}>{delegate.location}</Text>
              </View>
              <View style={styles.metadataItem}>
                <TimeWIcon size={20} color={colors.primary} />
                <Text style={styles.metadataText}>{delegate.time}</Text>
              </View>
              {delegate.workshopNumber && (
                <View style={styles.metadataItem}>
                  <WorkshopIcon size={20} color={colors.primary} />
                  <Text style={styles.metadataText}>{delegate.workshopNumber}</Text>
                </View>
              )}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <Text style={styles.themeText}>{delegate.theme}</Text>
          </View>

          {/* Brief Overview Section */}
          <View style={styles.section}>
            <Text style={styles.sectionHeading}>Brief Overview:</Text>
            <Text style={styles.sectionText}>{delegate.overview}</Text>
          </View>
        </View>
      </ScrollView>
    </View>
...
export default DelegateListDetails;
